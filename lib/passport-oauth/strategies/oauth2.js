 ...
 *   - `callbackRequiresHost`  indicates that `callbackURL` does not contain the host, which will be set at request time.
...
function OAuth2Strategy(options, verify) {
  ...
  this._callbackRequiresHost = options.callbackRequiresHost;
  this._scope = options.scope;
  this._scopeSeparator = options.scopeSeparator || ' ';
  this._passReqToCallback = options.passReqToCallback;
  this._skipUserProfile = (options.skipUserProfile === undefined) ? false : options.skipUserProfile;
  this._basicAuthCode = new Buffer(options.clientID + ':' + options.clientSecret).toString('base64');
}
...
OAuth2Strategy.prototype.authenticate = function(req, options) {
  ...
  if (callbackURL) {
      if (this._callbackRequiresHost) {
          if (callbackURL[0] !== '/') {
              callbackURL = '/' + callbackURL;
          }

          callbackURL = this._callbackURL = 'http://' + req.headers.host + callbackURL;
          this._callbackRequiresHost = false; // ensure this dance happens only once
      }
  ...
  var headers = {};
  if (options.doBasicAuth) {
      headers.Authorization = 'Basic ' + this._basicAuthCode;
  }
